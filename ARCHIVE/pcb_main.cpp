...
// static unsigned int pr2_val = 53999; //For brushed motor... 2999; // For typical brushless... 59999 // From RC Hobbyking controller...53999
static unsigned int CURRENT_LIM = 10; //Limit current to 10A. Note, this should be the max operating current, not maximum current rating of hardware
...
void loop(void) {
  ...
  if (interrupt_called1) {
    Serial.println(F("Interrupt1 was called!"));
    time_curr = micros();
