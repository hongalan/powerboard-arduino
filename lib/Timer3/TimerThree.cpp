...
//AH_NOTE: This function has been overloaded to provide higher resolution PWM settings
void TimerThree::setPwmDuty(char pin, float duty)
{
  unsigned long dutyCycle = pwmPeriod;
  dutyCycle = (unsigned long)((float)(dutyCycle)*duty);
  dutyCycle >>= 10;
  if(pin == 5) OCR3A = dutyCycle;
  if(pin == 2) OCR3B = dutyCycle;
  if(pin == 3) OCR3C = dutyCycle;
